...
class HrLoan(models.Model):
    ...
    # is_employee_manager = fields.Boolean(compute="is_employee_manager_chk", default=False)
    is_hr_manager = fields.Boolean(compute="is_hr_manager_chk", default=False)
    responsible_approve_id = fields.Many2one('res.users', string='Responsible for Approve', required=False,
                                             domain=lambda self: [('groups_id', '=', self.env.ref(
                                                 'hr_loan.group_loan_request_approver').id)])
    date = fields.Date(string="Date", default=fields.Date.today(), readonly=True, help="Date")
    employee_id = fields.Many2one('hr.employee', string="Employee", required=True, help="Employee",
                                  domain=[('contract_id', '!=', False)])
    employee_manager_id = fields.Many2one('hr.employee', string='Employee Manager', required=True,
                                          related="employee_id.parent_id", readonly=False)
    hr_manager_id = fields.Many2one('res.users', string='HR Manager', required=False,
                                    related="employee_id.contract_id.hr_responsible_id", readonly=False)
    ...
    def check_total_amount(self):
        ...
        for line in self.loan_lines:
            total += line.amount
        if total != self.total_amount and total > 0.0:
            raise ValidationError(_('Total amount is %s must be equal to total amount %s') % (total, self.total_amount))

    def write(self, vals):
        res = super(HrLoan, self).write(vals)
        if vals.get('loan_lines', False):
    ...
    def action_submit(self):
        ...
        self.activity_schedule('mail.mail_activity_data_todo', user_id=self.hr_manager_id.id)

    # def action_employee_manager(self):
    #     self.write({'state': 'hr_manager'})
    #     self.activity_schedule('mail.mail_activity_data_todo', user_id=self.employee_manager_id.id)
    ...
    def is_responsible_approve_chk(self):
        for rec in self:
            ...
            if self.env.user.id == rec.responsible_approve_id.id and self.env.user.has_group(
                    'hr_loan.group_loan_request_approver'):
                rec.is_responsible_approve = True

    # def is_employee_manager_chk(self):
    #     for rec in self:
    #         rec.is_employee_manager = False
    #         if self.env.user.id == rec.employee_manager_id.user_id.id:
    #             rec.is_employee_manager = True

    def is_hr_manager_chk(self):
        for rec in self:
            rec.is_hr_manager = False
            if self.env.user.id == rec.hr_manager_id.id:
        ...
        if not self.employee_id.address_home_id.id:
            partner = self.env['res.partner'].create(
                {'name': self.employee_id.name,
                 'street': self.employee_id.address_id.street or False,
                 })
