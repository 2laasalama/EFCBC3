...
class UnpackingRFQ(models.Model):
    ...
    edit_date = fields.Date('تاريخ التحرير')
    ...
    def get_total_purchases(self):
        ...
        for order in self.requisition_id.purchase_ids.sorted('amount_total'):
            ...
            amount_total_txt = order.currency_id.with_context(lang='ar_001').amount_to_text(
                order.amount_total)
            orders.append({
                'order_id': order,
                'sequence': sequence,
    ...
    def get_report_data(self, records):
        ...
        for product in products:
            ...
            for line in order_lines.filtered(lambda x: x.product_id.id == product.id):
                add = True
                for item in res:
                    if item['product_id'] == product.id and item[
    ...
    def action_print(self):
        return self.env.ref('purchase_line_report.purchase_line_xlsx').report_action(
            self.line_ids.ids)
    ...
    def check_duplicate_approve(self):
        for rec in self:
            ...
            for line in accept_lines:
                if line.accept and accept_lines.filtered(
...
class UnpackingRFQLine(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', related='order_line_id.partner_id', string='المورد',
                                 readonly=True)
    product_id = fields.Many2one('product.product', related='order_line_id.product_id',
                                 string='الصنف', readonly=True)
    product_name = fields.Char(string='الصنف', readonly=True)
    price_unit = fields.Float(string='سعر الوحدة', related='order_line_id.price_unit',
                              readonly=True)
    product_qty = fields.Float(string='الكمية', related='order_line_id.product_qty', readonly=True)
    price_subtotal = fields.Monetary(related='order_line_id.price_subtotal', string='الاجمالى',
                                     readonly=True)
    price_total = fields.Monetary(related='order_line_id.price_total',
                                  string=' الاجمالى شامل الضريبة', readonly=True)
    date_order = fields.Datetime(related='order_line_id.date_order', string=' تاريخ الاستلام',
                                 readonly=True)
