odoo.define("queue_management.backend.dashboard", function (require) {
  ...
  var Dashboard = AbstractAction.extend({
    ...
    init: function (parent, context) {
      ...
      this.dashboards_templates = ["queue_management.dashboard_qms"];
      this.chartIds = {};
      this.graphs = [
        { name: "qms", type: "line" },
        { name: "qms", type: "pie" },
      ...
      var prom = this._rpc({
        ...
        params: {
          dateFrom:
            this.date_from.year() +
            "-" +
            (this.date_from.month() + 1) +
            "-" +
            this.date_from.date(),
          dateTo:
            this.date_to.year() +
            "-" +
            (this.date_to.month() + 1) +
            "-" +
            this.date_to.date(),
        },
      });
    ...
    render_graphs: function () {
      ...
      _.each(this.graphs, function (e) {
        ...
        if (e.type == "line") {
          self.render_graph(
            ".o_graph_" + e.name,
            self.dashboards_data[e.name].graph,
            chart_id
          );
        } else if (e.type == "pie") {
          self.render_pi_graph(
            ".o_graph_pi_" + e.name,
            self.dashboards_data[e.name].pi_graph,
            chart_id
          );
        }
        $(".o_graph_qms").hide();
      ...
      $(".o_chart_action").on("click", function (ev) {
        ...
        if (
          $(ev.currentTarget) &&
          $(ev.currentTarget).hasClass("fa-pie-chart")
        ) {
          $(".o_graph_pi_qms").show();
          $(".o_graph_qms").hide();
        } else {
    ...
    render_graph_analytics: function (client_id) {
      if (
        !this.dashboards_data.visits ||
        !this.dashboards_data.visits.ga_client_id
      ) {
        return;
      }
      ...
      gapi.analytics.ready(function () {
        ...
        window.onOriginError = function () {
          ...
          self.display_unauthorized_message(
    ...
    on_dashboard_action: function (ev) {
      ...
      if (actionExtra === "td_queue") {
        additionalContext = {
          search_default_tdqueue: true,
          search_default_today: true,
        };
      } else if (actionExtra === "served") {
        additionalContext = {
          search_default_served: true,
          search_default_today: true,
        };
    ...
    update_cp: function () {
      ...
      if (!this.$searchview) {
        this.$searchview = $(
          QWeb.render("queue_management.DateRangeButtons", {
            widget: this,
          })
        );
    ...
    handle_analytics_auth: function ($analytics_components) {
      ...
      if (!gapi.analytics.auth.getAuthResponse()) {
        this.display_unauthorized_message(
          $analytics_components,
          "not_connected"
        );
      } else if (
        gapi.analytics.auth.getAuthResponse() &&
        gapi.analytics.auth
          .getAuthResponse()
          .scope.indexOf("https://www.googleapis.com/auth/analytics") === -1
      ) {
    ...
    display_unauthorized_message: function ($analytics_components, reason) {
      $analytics_components.prepend(
        $(QWeb.render("website.unauthorized_analytics", { reason: reason }))
      );
    },
    ...
    make_analytics_calls: function ($analytics_components) {
      ...
      // 3. View Selector
      var $analytics_view_selector = $("<div>").addClass(
        "col-lg-12 o_properties_selection"
      );
      ...
      viewSelector.on("change", function (ids) {
        ...
        if (table_row_listener) {
          google.visualization.events.removeListener(table_row_listener);
        }
      });
      ...
      mainChart.on("success", function (response) {
        ...
        table_row_listener = google.visualization.events.addListener(
          chart,
          "select",
          function () {
            var options;
    ...
    analytics_create_components: function () {
      gapi.analytics.createComponent("ActiveUsers", {
        ...
        onChange_: function (delta) {
          ...
          if (valueContainer) {
            valueContainer.innerHTML = this.activeUsers;
          }
    ...
    format_number: function (value, type, digits, symbol) {
      if (type === "currency") {
        ...
        return (
          field_utils.format[type](value || 0, { digits: digits }) +
          " " +
          symbol
        );
      }
    ...
    render_monetary_field: function (value, currency_id) {
      ...
      var formatted_value = field_utils.format.float(value || 0, {
