...
class check_cycle_accs(models.TransientModel):
    ...
    name_debit = fields.Char(default="Please Confirm Debit Check", readonly=True)
    name_csreturn = fields.Char(default="Are you sure you want to return the checks to customer", readonly=True)
    name_vreturn = fields.Char(default="Are you sure you want to return the checks from Vendor", readonly=True)
        ...
        if 'action_wiz' in self.env.context:
            ...
            if self.env.context['action_wiz'] == 'deposit':
                ...
                for check in checks:
                    ...
                    debit_account = []
                    credit_account = []
                    debit_account.append({'account': self.journal_id.default_account_id.id, 'percentage': 100})
                    ...
                    self.sudo().env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                               ...
                                                               src_currency=company_id.currency_id,
                                                               amount=check.amount_comp_currency)
                ...
                for check in checks:
                    ...
                    if checkamt > 0.0:
                        amount ,credit_ml,debit_ml= self.sudo().env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                               debit_account=debit_account,
                                                               credit_account=credit_account,
                                                               src_currency=check.currency_id,
                                                               amount=checkamt)
                        check.amount_app +=amount
                    if check.open_amount == 0.0:
                        check.state = 'approved'
                        diff = check.amount_comp_currency - check.amount_app
                        if diff !=0:
                            debit_account = []
                            credit_account = []
                            acc = check.notes_rece_id.id
                            if check.under_collect_id:
                                acc = check.under_collect_id.id

                            exchange_journal = company_id.currency_exchange_journal_id
                            if diff > 0:
                                credit_account.append({'account': acc, 'percentage': 100})
                                debit_account.append({'account': company_id.expense_currency_exchange_account_id.id, 'percentage': 100})
                            else:
                                debit_account.append({'account': acc, 'percentage': 100})
                                credit_account.append({'account': company_id.income_currency_exchange_account_id.id, 'percentage': 100})
                                diff = diff*-1

                            move = {

                                'journal_id': exchange_journal.id,
                                'ref': 'Currency exchange rate difference Check' + check.check_number,
                                'company_id': company_id.id
                            }
                            move_line = {
                            ...
                            amount, credit_ml, debit_ml = self.sudo().env['create.moves'].create_move_lines(move=move,
                                                    move_line=move_line,
                                                    debit_account=debit_account,
                                                    credit_account=credit_account,
                                                    src_currency=company_id.currency_id,
                ...
                for check in checks:
                    ...
                    debit_account = []
                    credit_account = []
                    credit_account.append({'account': check.under_collect_jour.default_account_id.id, 'percentage': 100})
                    if check.notes_rece_id:
            ...
            elif self.env.context['action_wiz'] == 'vr_return':

                checks = self.env['check.management'].search([('id', 'in', self.env.context['active_ids'])])

                for check in checks:
                    if check.nom_pay_id:
                        pay = check.nom_pay_id
                        journal_id = check.nom_pay_id.payment_method.id
                        move = {

                            'journal_id': journal_id,
                            'ref': 'Returning Check number ' + check.check_number + ' from Vendor',
                            'company_id': company_id.id,
                        ...
                        if check.investor_id:
                            credit_account= [{'account': pay.account_id.id, 'percentage': 100,
                                              'analytic_id': pay.analytic_id.id,
                                              'analytic_tag_id': pay.analytic_tag_id.id, }]
                            debit_account = [{'account': pay.payment_method.default_account_id.id, 'percentage': 100}]
                ...
                checks = self.env['check.management'].search([('id', 'in', self.env.context['active_ids'])])
                for check in checks:
                    if not check.book_id.journal_id:
                        raise exceptions.ValidationError(_('Please provide the bank account'))
                    move = {

                        'journal_id': check.book_id.journal_id.id,
                    ...
                    debit_account = []
                    credit_account = []
                    credit_account.append({'account': check.book_id.journal_id.default_account_id.id, 'percentage': 100})
                    ...
                    amount, credit_ml, debit_ml=self.sudo().env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                               ...
                                                               amount=check.amount)
                    check.state = 'debited'
                    diff = check.amount_comp_currency - amount
                    if diff != 0:
                        exchange_journal = company_id.currency_exchange_journal_id
                        if diff < 0:
                            ...
                            debit_account.append(
                                {'account': company_id.expense_currency_exchange_account_id.id, 'percentage': 100})
                            diff = diff * -1
                        else:

                            credit_account = []
                            credit_account.append(
                                {'account': company_id.income_currency_exchange_account_id.id, 'percentage': 100})


                        move = {

                            'journal_id': exchange_journal.id,
                            'ref': 'Currency exchange rate difference ' + check.check_number,
                            'company_id': company_id.id
                        }
                        move_line = {
                ...
                for check in checks:
                    if check.nom_pay_id:
                        ...
                        if check.investor_id:
                            ...
                            credit_account = [{'account': check.notespayable_id.id,'analytic_id': pay.analytic_id.id,
                                              'analytic_tag_id': pay.analytic_tag_id.id,'percentage' : 100}]
                            self.env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                                       debit_account=debit_account,
                                                                       credit_account=credit_account,
                                                                       src_currency=company_id.currency_id,
